...
def gen_category_scatter_data():
    """多类别相关性散点图: X, Y, Y_err（误差）, Group（分组）"""
    np.random.seed(42)
    n = 50
    data = []
    groups = ['Group1', 'Group2']
    
    # Group1: y = 0.921x - 0.006
    x1 = np.random.uniform(0, 1.6, n)
    y1 = 0.921 * x1 - 0.006 + np.random.normal(0, 0.12, n)
    err1 = np.random.uniform(0.05, 0.15, n)
    
    # Group2: y = 0.785x + 0.088
    x2 = np.random.uniform(0, 1.6, n)
    y2 = 0.785 * x2 + 0.088 + np.random.normal(0, 0.10, n)
    err2 = np.random.uniform(0.05, 0.12, n)
    
    for x, y, err in zip(x1, y1, err1):
        data.append({'X': x, 'Y': y, 'Y_err': err, 'Group': 'Group1'})
    for x, y, err in zip(x2, y2, err2):
        data.append({'X': x, 'Y': y, 'Y_err': err, 'Group': 'Group2'})
...
def gen_matrix_bubble_data():
    """矩阵气泡图: Row, Col, Value（颜色）, Size（大小）"""
    np.random.seed(42)
    ...
    for r in rows:
        for c in cols:
            data.append({
                'Row': r, 
                'Col': c, 
                'Value': round(np.random.uniform(52, 150), 1),   # 颜色映射
                'Size': round(np.random.uniform(75, 150), 1)     # 大小映射
            })
    return pd.DataFrame(data)
